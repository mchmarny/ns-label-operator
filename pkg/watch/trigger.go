...
func (w *NsWatch) apply(ns *corev1.Namespace) error {
	...
	for i, y := range w.manifests {
		if err := w.serverApply(ctx, ns, y); err != nil {
...
func (w *NsWatch) serverApply(ctx context.Context, ns *corev1.Namespace, deploymentYAML string) error {
	if ns == nil {
		return errors.New("nil namespace")
	}
	if deploymentYAML == "" {
	...
	dc, err := discovery.NewDiscoveryClientForConfig(w.cfg)
	if err != nil {
		return errors.Wrapf(err, "error creating discovery client using: %v", w.cfg)
	}

	mapper := restmapper.NewDeferredDiscoveryRESTMapper(memory.NewMemCacheClient(dc))
	dyn, err := dynamic.NewForConfig(w.cfg)
	if err != nil {
		return errors.Wrapf(err, "error creating dynamic client using: %v", w.cfg)
	}
